...
1. **Landing Page**  
   ...
   Users are presented with a clean and minimal interface to begin uploading their tax documents.
...
4. **Send the Documents to Server**  
   ![Send the documents to server](demo/4.png)  
   The user initiates document processing by sending them to the backend API.

5. **File Upload Success**  
   ![File upload success](demo/5.png)  
   The backend successfully receives and parses the uploaded tax documents.

6. **Retrieve Form 1040**  
   ![Retrieve Form 1040](demo/6.png)  
   Once processed, the frontend fetches the completed Form 1040 from the backend.

7. **Preview of Form 1040**  
   ![Preview of Form 1040](demo/7.png)  
   A preview of the generated Form 1040 is shown with tax fields filled based on extracted values.

8. **Download Form 1040**  
   ![Download Form 1040](demo/8.png)  
...
9. **Backend Docs Overview**  
   ![Backend API documentation](demo/9.png)  
   Interactive Swagger UI for browsing and testing backend endpoints.

10. **Upload Endpoint**  
    ![Upload Documents API](demo/10.png)  
    Accepts tax documents (W-2, 1099-NEC, 1099-INT) for extraction and calculation.

11. **Get Form 1040 Endpoint**  
    ![Get Form 1040 API](demo/13.png)  
